...
npm install magpie-project --save
```

the npm installation process creates a folder (named `node_modules`) in your experiment's directory where the npm dependencies are stored. After successfully installing \_magpie, the `node_modules` folder should contain `magpie-project` and its dependencies `jquery` and `csv-js`.

2. Add \_magpie

The `magpie-project` folder includes the following three files that you can add to your experiment:
...
`<script src='path/to/node_modules/magpie-project/magpie.full.js'></script>`
...
`<script src='path/to/node_modules/magpie-project/magpie.js'></script>`
...
#### Option 2: Download the magpie-project (not reccommended)
...
* `views_seq` - an array of view objects in the sequence you want them to appear in your experiment. [more info](https://github.com/magpie-project/magpie-base#views-in-_magpie)
* `deploy` - an object with information about the deploy methods of your experiment. [more info](https://github.com/magpie-project/magpie-base#deploy-configuration)
* `progress_bar` - an object with information about the progress bars in the views of your experiment. [more info](https://github.com/magpie-project/magpie-base#progress-bar)
...
[Here](https://github.com/magpie-project/departure-point) you can find a minimal experiment created with \_magpie, you can use this template as a starting point for your experiment. [Showroom](https://github.com/magpie-project/showroom) is an experiment which demonstrates most of \_magpie's functionalities including most views, hooks and the canvas-api.
...
Use `npm run watch` command from the `magpie-project` folder to start a process which watches for changes in the files in `src` and builds (updates) `magpie.js` and `magpie.full.js`. This commands builds both `magpie.js` and `magpie.full.js` when a file  in `src` is saved.

##### Option 2: Make changes to the files and then build the \_magpie files

Run `npm run concat` from the `magpie-project` folder. This command builds both `magpie.js` and `magpie.full.js`.
...
Run `npm publish` from the `magpie-project` folder to publish the new version of \_magpie.
